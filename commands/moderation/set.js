...
exports.run = async (client, message, [action, key, ...value], level) => { // eslint-disable-line no-unused-vars
  ...
  if (action === "edit") {
    ...
    if (!key) return message.reply("Onii chan please specify a key to edit");
    // User must specify a key that actually exists!
    if (!defaults[key]) return message.reply("Onii chan this key does not exist in the settings");
    const joinedValue = value.join(" ");
    // User must specify a value to change.
    if (joinedValue.length < 1) return message.reply("Onii chan please specify a new value");
    // User must specify a different value than the current one.
    if (joinedValue === settings[key]) return message.reply("Onii chan this setting already has that value!");
  ...
  if (action === "del" || action === "reset") {
    if (!key) return message.reply("Onii chan please specify a key to reset.");
    if (!defaults[key]) return message.reply("Onii chan this key does not exist in the settings");
    if (!overrides[key]) return message.reply("Onii chan this key does not have an override and is already using defaults.");
  ...
  if (action === "get") {
    if (!key) return message.reply("Onii chan please specify a key to view");
    if (!defaults[key]) return message.reply("Onii chan this key does not exist in the settings");
    const isDefault = !overrides[key] ? "\nOnii chan this is the default global default value." : "";
...
exports.help = {
  ...
  usage: "<prefix>set <view/get/edit> <key> <value>",
  option: ""
};
